...
| 2026-05-07 20:49 UTC | regression | agent.yaml | 3/3 | — | spin_360:P; go_to_random_and_back:P; personality_intro:P | — |
| 2026-05-07 20:55 UTC | demo bundle | agent.yaml | 3/3 + smoke | — | Full recommended bundle landed: (1) Mac+Ollama latency measured — decompose 4.5s simple / 13.8s compound / chit-chat cue 42s end-to-end → 5 Hz tick rate REQUIRES ZGX (fallback profile D needs tick_hz dropped to 1); (2) `runs/submit-backup.jsonl` captured (obstacle-corridor grade A) and tracked via gitignore exception; (3) personality_intro deterministic plan added (remember + emote(wave) + speak('Just Build') + emote(nod)); regression suite now mirrors runtime routing (classify_cue_smart → generate_plan, decompose only for compound) — spin_360 now 0ms (was 4500ms); (4) DEMO_SCRIPT updated to 3-cue chain opening with intro; (5) `scripts/demo_history.sh` for closing slide; (6) `hack commentate` shipped — local-LLM-narrated JSONL events, smoke-tested on submit-backup.jsonl, coherent plain-English sentences for non-engineer judges. | Updated PREP_TODO line 71 with latency numbers; added cut-list rule for commentator (kill if slow); REF.md cut-list T+1:45 row points at submit-backup.jsonl. Single commit covering 8 files + 2 new (`commentator.py`, `demo_history.sh`). |
